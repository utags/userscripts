...
;(() => {
  ...
  function updateFavicon(count) {
    ...
    if (count === 0) {
      ...
      lastGeneratedFavicon = link.href
      document.head.append(link)
      return
    }
    if (
  ...
  function handleKeyDown(e) {
    ...
    for (const shortcut of currentBlockedShortcuts) {
      ...
      e.stopPropagation()
      console.info(
        '[2libra-plus] \u{1F6AB} \u5DF2\u5C4F\u853D\u5FEB\u6377\u952E:',
        key
      )
      return
    }
  }
  function initBlockKeyboardShortcuts(getSettings2) {
    if (hasInitialized) return
    hasInitialized = true
    updateBlockedShortcuts(getSettings2().blockedShortcuts)
    globalThis.addEventListener('keydown', handleKeyDown, true)
  }
  function runBlockKeyboardShortcuts(getSettings2) {
    updateBlockedShortcuts(getSettings2().blockedShortcuts)
  }
  function updateBlockedShortcuts(shortcutsStr) {
    if (!shortcutsStr) {
  ...
  var rewardProbability = DEFAULT_SETTINGS.rewardProbability
